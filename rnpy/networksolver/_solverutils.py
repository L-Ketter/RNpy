def get_Ax(nw, x):
    ...
    """
    k = nw.k_arrs
    Ax = k['sum'].copy() * x[nw.u_slices['center']]
    for d in nw.directions:
        Ax -= k[d] * x[nw.u_slices[d]]
    return Ax

def get_r(nw, x, rhs=None):
    """
    ...
    """
    Ax = get_Ax(nw, x)
    r = rhs - Ax if rhs is not None else -Ax
    return r
...
def get_r_scaled_L1(nw, x, rhs=None):
    ...
    """
    r_scaled = get_r(nw, x=x, rhs=rhs) * nw.k_arrs['sum_inv']
